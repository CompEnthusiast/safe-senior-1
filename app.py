...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///safe3.db'
...
def health_records():
    ...
    records = HealthResponse.query.filter_by(user_id=user_id).order_by(HealthResponse.timestamp.desc()).limit(3).all()
    
    averages = {}
    if records:
        for i in range(1, 11):
            key = f'q{i}'
            yes_count = sum(getattr(record, key) == 'yes' for record in records)
            avg_value = 'yes' if yes_count >= 2 else 'no'
            averages[key] = avg_value

    return render_template('health_records.html', records=records, averages=averages)
